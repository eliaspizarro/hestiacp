...
check_wget_curl() {
	...
	if [ -e '/usr/bin/wget' ]; then
		...
		# 	fi
		# else
		wget -q https://raw.githubusercontent.com/eliaspizarro/hestiacp/refs/heads/release/install/hst-install-$type.sh -O hst-install-$type.sh
		if [ "$?" -eq '0' ]; then
	...
	if [ -e '/usr/bin/curl' ]; then
		...
		# 	fi
		# else
		curl -s -O https://raw.githubusercontent.com/eliaspizarro/hestiacp/refs/heads/release/install/hst-install-$type.sh
		if [ "$?" -eq '0' ]; then
			bash hst-install-$type.sh $*
			exit
		else
